...
/** Lets a vendor pick which part of their Logo photo survives the wide
 * flyer-card crop, instead of it always being dead-center. Drag/click
 * anywhere on the full photo to move the focal point; the strip below
 * shows the real flyer crop live as you move it. */
export function LogoCropEditor({
  ...
  async function handleSave() {
    ...
    const { error: updateError } = await supabase
      .from("vendors")
      .update({ logo_focal_x: focal.x, logo_focal_y: focal.y })
      .eq("id", vendorId);
    setSaving(false);
    if (updateError) {
    ...
    <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/80 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold text-slate-900">Position Your Flyer Photo</h3>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
        ...
        <p className="mt-1 text-xs text-slate-500">
          Click or drag anywhere on the photo to move what shows on your flyer card.
        </p>
        ...
        >
          ...
          <img src={logoUrl} alt="Your logo" className="pointer-events-none h-full w-full object-contain" />
          <div
