...
function FeatureCard({
  ...
  art: ReactNode;
  cta: string;
  ctaHref: { kind: "route" | "static"; href: string };
  extra?: ReactNode;
}) {
  return (
    <article className="cp-glass flex flex-col p-3 sm:p-4">
      <div className="cp-content flex h-full flex-col">
        ...
        <div className="cp-matte-panel relative mt-3 min-h-[140px] flex-1 overflow-hidden sm:min-h-[170px]">
          {art}
        </div>

        <div className="mt-3">
          <CpLink
            to={ctaHref.kind === "route" ? route(ctaHref.href) : staticPage(ctaHref.href)}
            className="cp-btn-pop cp-press flex w-full items-center justify-center gap-2 px-4 py-2.5"
          >
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-3 sm:gap-4">
      <FeatureCard
        ...
        title="Corkboard"
        icon={<IconMapPin className="h-5 w-5 text-slate-200" />}
        copy={["Different boards for every flyer.", "Each city. Easy to find."]}
        art={<CorkboardArt />}
        cta="Digital Flyers"
        ctaHref={{ kind: "static", href: CORKBOARD_URL }}
      />
      ...
      <FeatureCard
        ...
        art={<MapArt />}
        cta="Open to Locate"
        ctaHref={{ kind: "static", href: MAP_URL }}
        extra={
          <a
            href={ADD_PIN_URL}
            className="cp-press mt-2 flex items-center justify-center gap-1.5 py-1 text-[12px] font-semibold text-slate-300 hover:text-white"
          >
      ...
      <FeatureCard
        eyebrow="Local"
        title="Businesses"
        icon={<IconCompass className="h-5 w-5 text-slate-200" />}
        copy={["Show near me", "local happenings."]}
        art={<BoothArt />}
        cta="Main Directory"
        ctaHref={{ kind: "route", href: "/vendors" }}
      />
    </div>
  );
}
