 ...
 * "Explore Your City".
 *
 * CityPinned is in beta with Downtown San Jose live today, so the search
 * resolves what you type against the cities that actually have a board and
 * says so plainly when it doesn't match — it routes to the existing board
 * rather than pretending to query a service that isn't there yet.
 */
const LIVE_CITIES = [
  {
    name: "San Jose",
...
export function CitySearch() {
  ...
  return (
    <section className="cp-glass p-4 sm:p-5" aria-labelledby="cp-explore-city">
      <div className="cp-content">
        <div className="flex items-center gap-2.5">
          <span className="cp-matte-panel flex h-9 w-9 items-center justify-center rounded-full">
            <IconMapPin className="cp-content h-5 w-5 text-slate-100" />
          </span>
          <h2 id="cp-explore-city" className="cp-chrome-text text-lg font-bold sm:text-xl">
            Explore Your City
          </h2>
        </div>
        ...
        <form onSubmit={submit} className="mt-3 flex items-center gap-2">
          ...
          <button
            type="submit"
            aria-label="Search"
            className="cp-btn cp-press flex h-12 w-12 shrink-0 items-center justify-center"
          >
        ...
        {notFound && (
          <p role="status" className="cp-matte-panel mt-3 px-4 py-3 text-[13px] leading-snug text-slate-300">
