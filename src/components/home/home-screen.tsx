...
function categoryIcon(category: Category) {
  ...
  if (/season|holiday|winter|summer/.test(key)) return <IconLeaf className={cls} />;
  if (/farm|market|produce|grocer/.test(key)) return <IconBasket className={cls} />;
  if (/shop|retail|boutique|craft|artisan|maker/.test(key)) return <IconBag className={cls} />;
  if (/food|restaurant|cook|eat|bake|dessert|drink|coffee/.test(key))
    return <IconUtensils className={cls} />;
  if (category.icon) {
    return (
      <span aria-hidden="true" className="text-sm leading-none grayscale">
        {category.icon}
      </span>
    );
  }
    ...
    <div className="cp-screen min-h-screen">
      ...
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          <CitySkyline className="h-full w-full" seed={11} />
          <div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/30 to-[#050506]" />
        </div>

        <div className="relative mx-auto w-full max-w-[560px] px-3 pb-6 pt-5 sm:max-w-3xl sm:px-5 lg:max-w-6xl">
          <div className="flex items-start gap-3 sm:gap-6">
            <UltimatePin
              className="h-40 w-auto shrink-0 sm:h-56 lg:h-72"
              glow
              title="CityPinned Ultimate Pin"
            />
            <div className="min-w-0 pt-2">
              <h1 className="text-[26px] font-extrabold leading-[1.1] tracking-tight text-white sm:text-4xl lg:text-5xl">
                See local flyers,
                <br />
              ...
              <div className="cp-glass mt-4 inline-flex max-w-full items-center gap-2 px-3 py-2 sm:px-5 sm:py-2.5">
                <span className="cp-content flex items-center gap-2">
                  <IconSparkle className="h-3.5 w-3.5 shrink-0 text-slate-200" />
                  <span className="cp-chrome-text font-mono text-[10px] font-bold uppercase tracking-[0.14em] sm:text-sm">
      ...
      <div className="mx-auto w-full max-w-[560px] px-3 sm:max-w-3xl sm:px-5 lg:max-w-6xl">
        ...
        {/* ------------------------------------------------------- CATEGORIES */}
        <section className="pt-7" aria-labelledby="cp-categories-heading">
          ...
          <ul className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5 sm:gap-3">
            {shownCategories.map((category) => (
              <li
                key={category.id}
                className="cp-glass flex items-center justify-center gap-2 px-3 py-2.5"
              >
                <span className="cp-content flex items-center gap-2">
            ...
            <li className={shownCategories.length ? "" : "col-span-2 sm:col-span-5"}>
              <Link
                href="/vendors"
          ...
          <div className="cp-glass p-4 sm:p-5">
            <div className="cp-content flex flex-col gap-4 sm:flex-row sm:items-center">
              <div className="flex flex-1 items-start gap-3">
                <IconGift className="mt-0.5 h-7 w-7 shrink-0 text-slate-200" />
                <div>
              ...
              <a
                href="#pricing"
                className="cp-btn-pop cp-press flex items-center justify-center gap-2 px-5 py-3 sm:shrink-0"
              >
                <span className="cp-content flex items-center gap-2">
        ...
        <section className="pt-3" aria-label="Why CityPinned">
          <div className="cp-glass p-1.5">
            <div className="cp-content grid grid-cols-2 divide-white/10 sm:grid-cols-4 sm:divide-x">
              <Benefit
                icon={<IconGift className="h-6 w-6 text-slate-200" />}
                title="100% Free"
                lines={["No account", "needed to browse."]}
              />
              <Benefit
                icon={<IconMapPin className="h-6 w-6 text-slate-200" />}
                title="Live Pins"
                lines={["Events, vendors &", "places updated constantly."]}
              />
              <Benefit
                icon={<IconLayers className="h-6 w-6 text-slate-200" />}
                title="All in One"
                lines={["Board, Map &", "Directory in one place."]}
              />
              <Benefit
                icon={<IconPeople className="h-6 w-6 text-slate-200" />}
                title="For Everyone"
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 sm:gap-5">
            ...
            {!tiers.length && (
              <p className="cp-glass px-4 py-5 text-center text-sm text-slate-400 sm:col-span-2">
                <span className="cp-content">Pricing tiers are being set up — check back soon.</span>
              </p>
            )}
          </div>
        </section>
        ...
        <section className="pt-8">
          <div className="cp-glass relative overflow-hidden p-4 sm:p-6">
            <div className="cp-art">
              <CitySkyline className="absolute inset-y-0 right-0 h-full w-[85%]" seed={23} reflection={false} />
              <div className="absolute inset-0 bg-gradient-to-r from-[#050506] via-[#050506]/85 to-transparent" />
            </div>
            <div className="cp-content flex items-center gap-3">
              <div className="min-w-0 flex-1">
                ...
                <Link
                  href="/vendor/signup"
                  className="cp-btn-pop cp-press mt-4 inline-flex items-center gap-2 px-6 py-3"
                >
                  <span className="cp-content flex items-center gap-2">
              ...
              <UltimatePin className="h-24 w-auto shrink-0 sm:h-36" glow />
            </div>
          </div>
        </section>
